import {join, fromFileUrl} from "https://deno.land/std@0.201.0/path/mod.ts";
import {R, z} from "./deps.ts"
import { Statement, expression, serialize, Expression } from './code-generator.ts'
import {
    createHubspotAxios,
...
const main = async () => {
    const statements: Statement[] = [
        { type: 'import', importNames: ['z'], module: 'https://deno.land/x/zod@v3.22.2/mod.ts' }
    ]
    ...
    statements.push({
        ...
        expression: expression().object({
            ...
            definition: expression().object(R.fromPairs(hubspotObjectTypesToDownload.map(name => [name, expression().var(name + 'Definition').finishExpr()] as const)))
        }),
        export: true,
    })
    ...
    await Deno.writeTextFile(debugPath, JSON.stringify(debugOutput, null, 4), {create: true});
